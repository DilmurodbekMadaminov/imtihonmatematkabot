...
export default function AdminPanel({ currentUserId, isAdmin }: { currentUserId?: number; isAdmin?: boolean }) {
  ...
  const [activeTab, setActiveTab] = useState<
    "stats" | "users" | "broadcast" | "channels" | "candidates" | "tests"
  >("stats");
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.success && data.isAdmin) {
          ...
          setAuthError(data.error || "Ushbu Telegram ID admin emas!");
      ...
      console.error(err);
      setAuthError("Tarmoq ulanishida xatolik.");
      setIsAuthenticated(false);
    } finally {
      setVerifying(false);
    }
  ...
  const fetchCandidates = async () => {
    try {
      ...
      const res = await adminFetch("/api/candidates");
      if (res.ok) {
        const data = await res.json();
        setCandidates(data);
      }
  ...
  const handleBroadcast = async (e: React.FormEvent) => {
    ...
    try {
      const res = await adminFetch("/api/broadcast", {
        method: "POST",
        body: JSON.stringify({
  ...
  const handleSendIndividual = async (e: React.FormEvent) => {
    ...
    try {
      const res = await adminFetch("/api/message-user", {
        method: "POST",
        body: JSON.stringify({
      ...
      if (res.ok) {
        ...
        alert(
          "Xabar yuborilmadi. Foydalanuvchi botni o'chirgan yoki bloklagan bo'lishi mumkin.",
        );
      }
    } catch (e) {
      console.error(e);
  ...
  const handleToggleBan = async (user: UserProfile) => {
    ...
    if (
      !window.confirm(
        `${user.firstName || user.id} foydalanuvchisini ${actionText} xohlaysizmi?`,
      )
    ) {
      return;
    }
    ...
    try {
      const res = await adminFetch("/api/users/ban", {
        method: "POST",
        body: JSON.stringify({
          userId: user.id,
  ...
  const handleCandidateStatus = async (
    candidate: Candidate,
    ...
    try {
      const res = await adminFetch("/api/candidates/status", {
        method: "POST",
        body: JSON.stringify({
  ...
  const exportToExcel = () => {
    ...
    link.setAttribute(
      "download",
      `tizim_foydalanuvchilari_${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  const totalTests = users.reduce(
    (acc, curr) => acc + (curr.testsTaken || 0),
    0,
  );
  const averageScore =
    users.length > 0
      ? Math.round(
          users.reduce((acc, curr) => acc + (curr.averageScore || 0), 0) /
            users.length,
        )
      : 0;
  ...
  if (isAuthenticated === null) {
    return (
      <div className="bg-slate-900 text-white rounded-3xl border border-slate-800 shadow-2xl p-8 space-y-6 relative overflow-hidden min-h-[500px] flex flex-col items-center justify-center">
        <div className="absolute top-0 right-0 w-80 h-80 bg-indigo-500/10 rounded-full blur-3xl pointer-events-none"></div>
        <div className="flex flex-col items-center gap-4">
          <RefreshCw className="text-indigo-500 animate-spin" size={40} />
          <span className="text-slate-400 font-medium text-xs tracking-wider uppercase">
            Tizimga kirilmoqda...
          </span>
        </div>
      </div>
  ...
  if (isAuthenticated !== true) {
    return (
      <div className="bg-slate-900 text-white rounded-3xl border border-slate-800 shadow-2xl p-6 md:p-8 space-y-6 relative overflow-hidden min-h-[500px] flex flex-col justify-between">
        <div className="absolute top-0 right-0 w-80 h-80 bg-indigo-500/10 rounded-full blur-3xl pointer-events-none"></div>
        ...
        <div className="space-y-6">
          <div className="text-center space-y-2 py-4 border-b border-slate-800/80">
            <span className="bg-indigo-950/80 text-indigo-400 border border-indigo-800/60 rounded-full px-3 py-1 text-[10px] font-bold tracking-widest uppercase inline-block">
            ...
            <h2 className="text-2xl font-extrabold text-white tracking-tight flex items-center justify-center gap-2">
              <Shield className="text-indigo-500" size={26} />
              Admin Panelga Kirish
            </h2>
            <p className="text-xs text-slate-400 max-w-sm mx-auto">
              Botni xavfsiz boshqarish uchun shaxsiy Telegram ID raqamingizni
              kiriting va tasdiqlang.
            </p>
          </div>
          ...
          >
            <div className="space-y-1.5">
              ...
              <input
                type="text"
                placeholder="Masalan: 512345678"
                value={inputTelegramId}
                onChange={(e) => setInputTelegramId(e.target.value)}
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 text-white text-center font-mono tracking-wider placeholder-slate-600"
                required
              />
            </div>

            {authError && (
              <div className="text-center text-xs text-red-400 bg-red-950/40 border border-red-950 px-3 py-2 rounded-lg">
                ⚠️ {authError}
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={verifying}
              className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-800/50 text-white py-3 px-4 rounded-xl font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition duration-200 cursor-pointer disabled:cursor-not-allowed"
            >
              {verifying ? (
                <>
        ...
        <div className="text-center text-[10px] text-slate-500 py-2">
          Ushbu panel faqatgina ruxsat etilgan Telegram ID egalari uchun ochiq.
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-4">
        <div>
          <span className="bg-indigo-950/80 text-indigo-400 border border-indigo-800/60 rounded-full px-3 py-1 text-[10px] font-bold tracking-widest uppercase inline-block mb-1">
            🖥️ Tizim Boshqaruvchisi {adminUser ? `[ID: ${adminUser.id}]` : ""}
          </span>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-2 border border-slate-800 hover:bg-red-950/40 hover:border-red-900/40 text-slate-400 hover:text-red-400 rounded-xl transition duration-150 text-xs font-bold shrink-0 cursor-pointer"
            title="Chiqish"
          >
            Chiqish
          </button>
        ...
        <AnimatePresence mode="wait">
            ...
            >
              <div>
                <h3 className="text-sm font-bold text-slate-300 flex items-center gap-1.5 uppercase tracking-wider mb-4 border-b border-slate-900 pb-2">
                  <Activity size={16} className="text-indigo-400" /> Tizim
                  Statistikasi & Holati
                </h3>
              ...
              <div className="bg-slate-900/40 p-4 rounded-xl border border-slate-850/80 text-xs text-slate-400 mt-4 leading-relaxed">
                ...
                Ushbu Admin paneli maxsus bot arxitekturasi asosida tayyorlandi.
                Pastdagi kalkulyator tugmachalari orqali xabar yozish, majburiy
                kanallarni boshqarish va a'zolarni bloklash bilan tizimni to'liq
              ...
              <div className="space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-900 pb-2">
                  <h3 className="text-sm font-bold text-slate-300 flex items-center gap-1.5 uppercase tracking-wider">
                    <Users size={16} className="text-indigo-400" /> A'zolar
                    Boshqaruvi ({filteredUsers.length})
                  </h3>
                  <div className="flex items-center gap-2">
                    <button
                ...
                <div className="max-h-[220px] overflow-y-auto border border-slate-900 rounded-xl divide-y divide-slate-900 custom-scrollbar">
                  {loadingUsers ? (
                    ...
                    filteredUsers.map((user) => (
                      <div
                        key={user.id}
                        className="p-3 hover:bg-slate-900/40 flex items-center justify-between text-xs transition gap-4"
                      >
                        <div className="min-w-0 flex-1">
                          <div className="font-bold text-slate-200 flex items-center gap-1.5 truncate">
                            {user.firstName || "Noma'lum"} {user.lastName || ""}
                            {user.isBanned && (
                              <span className="bg-red-950 text-red-400 border border-red-900 px-1.5 py-0.2 rounded text-[8px] font-bold">
                                BLOKLANGAN
                              </span>
                            )}
                          </div>
            ...
            >
              <div>
                <h3 className="text-sm font-bold text-slate-300 flex items-center gap-1.5 uppercase tracking-wider border-b border-slate-900 pb-2">
                  <Bell size={16} className="text-indigo-400 animate-pulse" />{" "}
                  Telegram Reklama Tarqatuvchi
                </h3>
                <p className="text-[10px] text-slate-500 mt-1">
                  Matnli reklamadan tashqari endi Suratli (Photo Ad) reklama
                  yuborish imkoni va rasm havolasini kiritish paneli ham
                  qo'shildi.
                </p>
              </div>
          ...
          {activeTab === "channels" && (
            ...
            >
              <div>
                <h3 className="text-sm font-bold text-slate-300 flex items-center gap-1.5 uppercase tracking-wider border-b border-slate-900 pb-2">
                  <Settings size={16} className="text-indigo-400" /> Obuna
                  Kanallari (Majburiy)
                </h3>
                <p className="text-[10px] text-slate-500 mt-1">
                  Foydalanuvchilar testlarni boshlashlari uchun ro'yxatdagi
                  kanallarga obuna bo'lishi shart.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2 max-h-[160px] overflow-y-auto custom-scrollbar border border-slate-900 p-2.5 rounded-xl">
                  {settings.channels.map((channel, idx) => (
                    <div
                      key={idx}
                      className="flex items-center justify-between bg-slate-900/60 border border-slate-850 rounded-lg p-2 text-xs"
                    >
          ...
          {activeTab === "candidates" && (
            ...
            >
              <div>
                <h3 className="text-sm font-bold text-slate-300 flex items-center gap-1.5 uppercase tracking-wider border-b border-slate-900 pb-2">
                  <Briefcase size={16} className="text-teal-400" /> Ustoz
                  Shogird Kandidatlari
                </h3>
                <p className="text-[10px] text-slate-500 mt-1">
                  Bot orqali ariza topshirgan o'qituvchilar va hamkorlar
                  ro'yxati.
                </p>
              </div>
                    ...
                    >
                      <div className="space-y-1">
                        ...
                        <div className="text-slate-400 text-[10px] flex items-center gap-3">
                          ...
                          <span>
                            💼{" "}
                            {cand.role === "teacher" ? "O'qituvchi" : "Hamkor"}
                          </span>
                        </div>
                      ...
                      <div className="flex gap-1.5 shrink-0">
                        <button
                          onClick={async () => {
                            if (!window.confirm("Tasdiqlashni xohlaysizmi?"))
                              return;
                            try {
                              const r = await adminFetch(
                                `/api/candidates/${cand.id}/status`,
                                {
                                  method: "POST",
                        ...
                        <button
                          onClick={async () => {
                            if (!window.confirm("Rad etishni xohlaysizmi?"))
                              return;
                            try {
                              const r = await adminFetch(
                                `/api/candidates/${cand.id}/status`,
            ...
            >
              <div>
                <div className="flex justify-between items-center border-b border-slate-900 pb-2">
                  <h3 className="text-sm font-bold text-slate-300 flex items-center gap-1.5 uppercase tracking-wider">
                    <BookOpen size={16} className="text-indigo-400" /> Test
                    Savollari Boshqaruvi
                  </h3>
                  <button
              ...
              {isAddingTest ? (
                <form
                  onSubmit={async (e) => {
                    ...
                    try {
                      const r = await adminFetch("/api/tests", {
                        method: "POST",
                        body: JSON.stringify({
                      ...
                      if (r.ok) {
                        alert(
                          "Test muvaffaqiyatli saqlandi va botga joylandi!",
                        );
                        setTestTitle("");
                        setTestQuestions([
                          {
                  ...
                  {testCategory === "matematika" && (
                    <div className="space-y-1 text-xs">
                      ...
                      >
                        ...
                        <option value="trigonometriya">
                          📁 Trigonometriya
                        </option>
                        <option value="matematik_analiz">
                          📁 Matematik Analiz
                        </option>
                      </select>
                    </div>
                  ...
                  <div className="border-t border-slate-850 pt-3 space-y-4">
                    <div className="flex justify-between items-center">
                      <h4 className="text-xs font-bold text-slate-300">
                        Savollar ({testQuestions.length} ta)
                      </h4>
                      <button
                        type="button"
                        onClick={() =>
                    ...
                    {testQuestions.map((q, qIndex) => (
                      <div
                        key={qIndex}
                        className="bg-slate-900/40 border border-slate-850 p-3 rounded-xl space-y-3 text-xs relative"
                      >
                        ...
                        <div className="space-y-1">
                          <label className="text-[10px] text-slate-505 font-bold">
                            Savol matni
                          </label>
                          <textarea
                            value={q.text}
                        ...
                        <div className="space-y-1">
                          <label className="text-[10px] text-slate-505 font-bold block">
                          ...
                          {q.options.map((opt: string, optIdx: number) => (
                            <div key={optIdx} className="space-y-1">
                              <label className="text-[9px] text-slate-500 font-bold">
                                {String.fromCharCode(65 + optIdx)} Variant
                              </label>
                              <input
                                type="text"
                                value={opt}
                ...
                <div className="flex-1 overflow-y-auto max-h-[280px] space-y-2 pr-1 custom-scrollbar border border-slate-900 p-3 rounded-2xl">
                  {loadingTests ? (
                    ...
                    <p className="text-xs text-slate-500 text-center py-8">
                      Hozircha qo'shimcha testlar kiritilmagan.
                    </p>
                  ) : (
                    customTests.map((t) => (
                      <div
                        key={t.id}
                        className="p-3 bg-slate-900/60 border border-slate-850 rounded-xl flex items-center justify-between text-xs gap-3"
                      >
                        <div>
                          <div className="font-bold text-white flex items-center gap-1.5 flex-wrap">
                            <span>{t.title}</span>
                        ...
                        <button
                          onClick={async () => {
                            if (!window.confirm("Rostdan o'chirilsinmi?"))
                              return;
                            try {
                              const r = await adminFetch("/api/tests/delete", {
                                method: "POST",
              ...
              <div className="bg-slate-950 border-b border-slate-850 px-6 py-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 mt-0.5">
                    Foydalanuvchi: {selectedUser.firstName || "Noma'lum"} (
                    {selectedUser.id})
                  </p>
                </div>
                <button
              ...
              <div className="p-6 space-y-4">
                <div className="bg-slate-950/80 p-4 border border-slate-850 rounded-xl space-y-2 text-xs">
                  ...
                  <div className="flex flex-col pt-1">
                    ...
                    <span className="text-slate-300 bg-slate-900/60 p-2.5 rounded border border-slate-900/80 leading-relaxed font-sans">
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">
                    Telegramga boradigan bildirishnoma matni (Ixtiyoriy
                    feedback)
                  </label>
                  <textarea
                    rows={2}
                ...
                <div className="flex gap-2 pt-2">
                  <button
                    onClick={() =>
                      handleCandidateStatus(selectedCandidate, "rejected")
                    }
                  ...
                  <button
                    onClick={() =>
                      handleCandidateStatus(selectedCandidate, "approved")
                    }
                    disabled={updatingCandidateId === selectedCandidate.id}
